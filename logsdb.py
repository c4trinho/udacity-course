...
def popular_posts():
	...
	cur = conn.cursor()
	cur.execute("SELECT articles.title, count(log) AS views FROM articles, log WHERE articles.slug = replace(log.path,'/article/','') GROUP BY articles.title ORDER BY views DESC LIMIT 3")
	return cur.fetchall()
	conn.close()
...
def popular_authors():
	...
	conn = psycopg2.connect("dbname=news user=vagrant")
	cur = conn.cursor()
	cur.execute("SELECT authors.name, count(log) AS views FROM articles, authors, log WHERE articles.slug = replace(log.path,'/article/','') AND articles.author = authors.id GROUP BY authors.name ORDER BY views DESC;")
	return cur.fetchall()
	conn.close()
